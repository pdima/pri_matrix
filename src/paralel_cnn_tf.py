...
class ParallelCNNDataset:
    def __init__(self, preprocess_input_func, batch_size, validation_batch_size=1):
        ...
        self.combine_batches = 4  # combine multiple batches in generator for parallel processing
    ...
    def generate_test(self):
        ...
        def load_clip(video_id):
            return self.load_train_clip(video_id, offset=2)
...
def train_initial(fold):
    model = build_model_resnet50(lock_base_model=True)
    ...
    model.fit_generator(
        ...
        callbacks=[
            ModelCheckpoint("../output/checkpoint-train_initial_8w-{epoch:03d}.hdf5",
                            verbose=1,
    ...
    np.save(f'../output/resnet50_w_initial_fold_{fold}_tf_last_w.npy', last_w)


def train_continue(fold):
    # model = build_model_resnet50(lock_base_model=False)
    # model.load_weights(f'../output/resnet50_w_initial_fold_{fold}_tf.h5')
    # w = model.layers[-1].get_weights()
    # del model
    # K.clear_session()

    w = np.load(f'../output/resnet50_w_initial_fold_{fold}_tf_last_w.npy')
    model = build_model_resnet50(lock_base_model=False)
    ...
    model.save_weights(f'../output/resnet50_w8_fold_{fold}.h5')
...
def check_model(weights):
    ...
    dataset = ParallelCNNDataset(preprocess_input_func=preprocess_input_resnet50, batch_size=1, validation_batch_size=1)
    batch_id = 0
