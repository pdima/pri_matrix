...
def build_model_resnet50(lock_layers_until):
    img_input = Input(INPUT_SHAPE, name='data')
    base_model = ResNet50(input_shape=INPUT_SHAPE[1:], include_top=False, pooling='avg')
...
class ParallelCNNDataset:
    ...
    def train_steps_per_epoch(self):
        ...
        return int(len(self.train_clips) / 16 // preprocess_batch_size * preprocess_batch_size // self.batch_size)
...
def train_initial(fold):
    ...
    model.save_weights(f'../output/resnet50_w_initial_fold_{fold}.h5')


def train_continue(fold):
    model = build_model_resnet50(lock_layers_until='activation_49')
    model.load_weights(f'../output/resnet50_w_initial_fold_{fold}.h5')
    ...
    for X, y in dataset.generate_test():
        ...
        for batch_frame in range(dataset.batch_size):
            for frame in range(3):
                print(y[batch_frame])
                plt.imshow(X[batch_frame, frame*5]/255.0)
                plt.show()
